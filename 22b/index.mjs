...
let result = 0;
for (const brick of bricks) {
  let supportedByMapCpy = structuredClone(supportedByMap);

  const desintegrated = new Set();

  const desintegrate = (brickId) => {
    for (const [brickId2, supportedByList] of Object.entries(
      supportedByMapCpy
    )) {
      if (supportedByList.delete(brickId) && supportedByList.size === 0) {
        ++result;
        desintegrate(brickId2);
      }
    }

    desintegrated.add(brickId);
  };

  desintegrate(brick.id);
}
console.log(result);
